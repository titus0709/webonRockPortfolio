...
const MsgIcon = ({ size = 20 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    <path d="M12 2C6.48 2 2 5.94 2 10.8c0 2.66 1.38 5.04 3.56 6.66-.12 1.15-.5 2.63-1.44 3.92a.5.5 0 00.56.77c2.06-.62 3.62-1.6 4.55-2.3.87.2 1.79.31 2.77.31 5.52 0 10-3.94 10-8.8S17.52 2 12 2z"/>
  </svg>
);
...
const Star = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="#F97316" aria-hidden="true">
...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    ::-webkit-scrollbar-thumb { background: #0B1F3A; border-radius: 10px; }
    ...
    .btn-gold {
      ...
      background: #EA580C; color: #0B1F3A;
      font-family: 'Inter', sans-serif;
    ...
    .btn-gold:hover { background: #C2410C; transform: translateY(-2px); box-shadow: 0 8px 28px rgba(234,88,12,.45); }
    ...
    .btn-ghost-green {
      ...
      border: 2px solid #0B1F3A; cursor: pointer;
      transition: background .18s, color .18s; text-decoration: none;
    }
    .btn-ghost-green:hover { background: #0B1F3A; color: #fff; }
    ...
    .pill-label-green {
      display: inline-flex; align-items: center; gap: 6px;
      background: rgba(11,31,58,.08); color: #0B1F3A;
      font-family: 'Inter', sans-serif;
      font-size: 11px; font-weight: 700; letter-spacing: .12em;
      text-transform: uppercase; padding: 5px 14px; border-radius: 100px;
    ...
    .card-lift:hover { transform: translateY(-5px); box-shadow: 0 20px 56px rgba(11,31,58,.12) !important; }

    /* Gold shimmer */
    .gold-shimmer {
      background: linear-gradient(90deg, #EA580C 0%, #FDBA74 40%, #EA580C 70%, #C2410C 100%);
    ...
    .stat-strip { display: flex; gap: 0; padding-top: 32px; border-top: 1px solid #E2E8F0; }
    ...
    @media (min-width: 601px) {
      .service-item:hover { padding-left: 16px !important; border-left: 3px solid #EA580C !important; }
    }

    /* Floating CTA */
    .floating-cta { position: fixed; bottom: 24px; right: 24px; z-index: 999; display: flex; align-items: center; gap: 10px;background: #EA580C; color: #0B1F3A; font-family: 'Inter', sans-serif; font-weight: 800; font-size: 14px; padding: 14px 24px; border-radius: 100px; box-shadow: 0 8px 32px rgba(234,88,12,.45); transition: transform .18s, box-shadow .18s; text-decoration: none; }
    .floating-cta:hover { transform: scale(1.05); box-shadow: 0 14px 44px rgba(234,88,12,.6); }
    @media (max-width: 600px) {
...
const AnnouncementBar = () => (
  <div className="announcement-bar" style={{ background: "#0B1F3A", color: "#fff", padding: "10px 24px", textAlign: "center", fontSize: "13px", fontWeight: 500, fontFamily: "'Inter',sans-serif" }}>
    🔥 Only 3 founding HVAC spots this quarter —&nbsp;
    <a href={BOOKING_URL} style={{ color: "#F97316", fontWeight: 700, textDecoration: "underline", textUnderlineOffset: "3px" }}>
...
const Ticker = () => (
  <div style={{ background: "#f8f6f0", borderBottom: "1px solid #E2E8F0", overflow: "hidden", padding: "11px 0" }}>
    <div style={{ display: "flex", gap: "60px", animation: "ticker 28s linear infinite", whiteSpace: "nowrap", width: "max-content" }}>
      {[...tickItems, ...tickItems].map((t, i) => (
        <span key={i} style={{ fontSize: "11.5px", fontWeight: 600, color: "#0B1F3A", letterSpacing: ".1em", textTransform: "uppercase", flexShrink: 0, display: "flex", alignItems: "center", gap: "8px", fontFamily: "'Inter',sans-serif" }}>
        ...
        <div style={{
          ...
          {[["Work", "#work"], ["Results", "#results"], ["Services", "#services"], ["Process", "#process"]].map(([l, h]) => (
            <a key={l} href={h} onClick={() => setMenuOpen(false)}
              style={{ fontSize: "16px", fontWeight: 600, color: "#172033", padding: "8px 0", borderBottom: "1px solid #E2E8F0" }}>
              {l}
            </a>
          ))}
          <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" className="btn-gold" style={{ justifyContent: "center", marginTop: "8px" }}>
            <PhIcon size={16} /> Book a Free Call
          </a>
        </div>
...
const Hero = () => (
  <section className="hero-section" style={{ background: "#fff", padding: "90px 28px 100px", position: "relative", overflow: "hidden" }}>
    <div style={{ position: "absolute", inset: 0, pointerEvents: "none", opacity: .4, backgroundImage: "radial-gradient(circle, #F9731622 1px, transparent 1px)", backgroundSize: "32px 32px" }} />
    <div style={{ position: "absolute", top: 0, right: 0, width: "420px", height: "420px", background: "#0B1F3A", borderRadius: "0 0 0 100%", opacity: .04, pointerEvents: "none" }} />
    ...
    >
      ...
      >
        ...
        <div className="hero-card animate-fade-up-4" style={{
          ...
          border: "1px solid #e5e7eb", boxShadow: "0 8px 32px rgba(11,31,58,.10)",
          position: "relative", overflow: "hidden", animation: "floatUp 5s ease-in-out infinite",
        }}>
          <div style={{ position: "absolute", top: 0, left: "28px", right: "28px", height: "3px", background: "#F97316", borderRadius: "0 0 4px 4px" }} />
          <p style={{ fontSize: "11px", fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", color: "#0B1F3A", marginBottom: "4px" }}>Case Study</p>
          <p style={{ fontSize: "15px", fontWeight: 800, color: "#172033", marginBottom: "16px", fontFamily: "'Inter', sans-serif" }}>Results We've Delivered</p>
          <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", fontSize: "11px", fontWeight: 700, background: "rgba(249,115,22,.12)", color: "#9A3412", border: "1px solid rgba(249,115,22,.3)", borderRadius: "20px", padding: "3px 10px", marginBottom: "14px" }}>
            🌡️ Lead generation for HVAC
          </span>
          ...
          <div style={{ position: "relative", marginBottom: "18px" }}>
            <div style={{ background: "#d1d5db", borderRadius: "8px 8px 0 0", padding: "7px 7px 0", boxShadow: "0 2px 8px rgba(0,0,0,.10)" }}>
              <div style={{ background: "#f3f6f4", borderRadius: "4px 4px 0 0", overflow: "hidden", height: "110px" }}>
                ...
                <div style={{ background: "#fff", height: "14px", display: "flex", alignItems: "center", padding: "0 8px", gap: "6px", borderBottom: "1px solid #e5e7eb" }}>
                  <div style={{ width: "32px", height: "6px", background: "#0B1F3A", borderRadius: "2px" }} />
                  {[18,22,16].map((w,i) => <div key={i} style={{ width: w, height: "5px", background: "#d1d5db", borderRadius: "2px" }} />)}
                  <div style={{ width: "22px", height: "6px", background: "#F97316", borderRadius: "2px", marginLeft: "auto" }} />
                </div>
                <div style={{ background: "#0B1F3A", height: "42px", display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 10px", gap: "4px" }}>
                  ...
                  <div style={{ width: "30px", height: "6px", background: "#F97316", borderRadius: "2px", marginTop: "2px" }} />
                </div>
            ...
            <div style={{ position: "absolute", top: "6px", right: 0, display: "flex", gap: "5px" }}>
              {[
                { score: 100, label: "SEO",    color: "#0B1F3A", pct: 1.00 },
                { score: 93,  label: "Access.", color: "#F97316", pct: 0.93 },
                { score: 94, label: "Speed",  color: "#1769AA", pct: 0.94 },
                ...
                return (
                  <div key={label} style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "5px 7px", display: "flex", flexDirection: "column", alignItems: "center", gap: "1px", boxShadow: "0 2px 6px rgba(0,0,0,.08)" }}>
                    ...
                    <span style={{ fontSize: "11px", fontWeight: 700, color: "#172033", lineHeight: 1 }}>{score}</span>
          ...
          <ul style={{ listStyle: "none", padding: 0, margin: "0 0 14px", display: "flex", flexDirection: "column", gap: "7px" }}>
           {["Consistent service calls from day one","Ranked for local search terms in 30 days","Perfect 100/100 SEO score on PageSpeed","Mobile-first, fast-loading design"].map(text => (
                <li key={text} style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "12.5px", color: "#64748B", lineHeight: 1.4 }}>
                  <span style={{ flexShrink: 0, width: "6px", height: "6px", borderRadius: "50%", background: "#0B1F3A", opacity: .7 }} />
                  {text}
                </li>
              ))}
          </ul>
          ...
          <div style={{ background: "#f0faf5", border: "1px solid #c6e5d6", borderRadius: "8px", padding: "10px 12px", marginBottom: "12px" }}>
            <p style={{ fontSize: "12px", color: "#166534", lineHeight: 1.5, margin: 0 }}>
              📈 <strong style={{ color: "#172033" }}>Result:</strong> Increased organic visibility and qualified lead generation within the first month.
            </p>
          </div>
          <a href="#work">
...
/* ─── CREDIBILITY STRIP ──────────────────────────────────────────── */
const CredStrip = () => (
  <section className="sm:hidden" style={{ background: "#0B1F3A", padding: "48px 28px" }}>
    <div style={{ maxWidth: "1160px", margin: "0 auto" }}>
      <Reveal>
        <p style={{ textAlign: "center", fontSize: "11px", fontWeight: 700, letterSpacing: ".14em", textTransform: "uppercase", color: "rgba(255,255,255,.4)", fontFamily: "'Inter',sans-serif", marginBottom: "32px" }}>
          Craft proven in India · Now serving U.S. HVAC contractors
        </p>
      </Reveal>
...
">
  {[
    ...
    >
      ...
      {item.s && (
        <div className="text-[#0B1F3A] font-bold text-sm mt-1">
          {item.s}
        </div>
      )}
        ...
        <h2 className="syne" style={{ fontSize: "clamp(1.9rem,4vw,3rem)", fontWeight: 800, color: "#172033", letterSpacing: "-.03em", lineHeight: 1.15, marginTop: "16px", marginBottom: "20px" }}>
          ...
          <Reveal key={title} type="reveal-scale" delay={i * 0.12}>
            <div className="card-lift" style={{ background: "#f9f9f7", border: "1px solid #E2E8F0", borderRadius: "12px", padding: "32px 28px", borderTop: "3px solid #F97316", height: "100%" }}>
              ...
              <p className="syne" style={{ fontSize: "16px", fontWeight: 700, color: "#172033", marginBottom: "10px" }}>{title}</p>
              <p style={{ fontSize: "14px", lineHeight: 1.75, color: "#64748B" }}>{body}</p>
            </div>
          </Reveal>
        ))}
      </div>
...
/* ─── WORK ───────────────────────────────────────────────────────── */
const Work = () => (
  <section id="work" style={{ background: "#f9f8f5", padding: "100px 28px", borderTop: "1px solid #E2E8F0" }}>
    <div style={{ maxWidth: "1160px", margin: "0 auto" }}>
      ...
      <div className="work-grid">
        {[
          ...
          <Reveal key={name} type="reveal-scale" delay={i * 0.12}>
            <div className="card-lift" style={{ background: "#fff", borderRadius: "12px", overflow: "hidden", border: featured ? "2px solid #F97316" : "1px solid #E2E8F0", boxShadow: featured ? "0 8px 32px rgba(249,115,22,.18)" : "0 2px 12px rgba(0,0,0,.04)", position: "relative" }}>
              {featured && (
                <div style={{ position: "absolute", top: "16px", right: "16px", background: "#F97316", color: "#0B1F3A", fontSize: "10px", fontWeight: 800, letterSpacing: ".1em", textTransform: "uppercase", padding: "4px 12px", borderRadius: "100px" }}>Best Results</div>
              )}
              <div style={{ background: "#0B1F3A", padding: "28px 28px 24px" }}>
                <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: ".12em", textTransform: "uppercase", color: "#F97316", marginBottom: "10px" }}>{tag}</p>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", borderBottom: "1px solid #E2E8F0" }}>
                {metrics.map(({ k, v }) => (
                  <div key={k} style={{ padding: "16px 12px", textAlign: "center", borderRight: "1px solid #E2E8F0" }}>
                    <p className="syne" style={{ fontSize: "20px", fontWeight: 800, color: "#0B1F3A" }}>{v}</p>
                    <p style={{ fontSize: "10px", color: "#64748B", marginTop: "3px", textTransform: "uppercase", letterSpacing: ".06em" }}>{k}</p>
                  </div>
                ))}
              </div>
              <div style={{ padding: "18px 24px", display: "flex", alignItems: "center", gap: "8px" }}>
                <span style={{ fontSize: "16px" }}>📈</span>
                <p style={{ fontSize: "13px", fontWeight: 600, color: "#172033" }}>{result}</p>
              </div>
            </div>
          </Reveal>
        ))}
      ...
      <div className="services-grid">
        <Reveal type="reveal-left">
          <div className="services-sticky" style={{ position: "sticky", top: "88px" }}>
            ...
            <h2 className="syne" style={{ fontSize: "clamp(1.9rem,3.5vw,2.6rem)", fontWeight: 800, color: "#172033", letterSpacing: "-.03em", lineHeight: 1.2, marginTop: "16px", marginBottom: "20px" }}>
              Everything Your Pipeline Needs to Stay Full
            </h2>
            <p style={{ fontSize: "15px", lineHeight: 1.75, color: "#64748B", marginBottom: "32px" }}>
              We don't sell "digital packages." We build lead pipelines — and we own the outcome with you.
            </p>
            <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" className="btn-gold">
              <PhIcon size={16} /> Talk Strategy
            </a>
          </div>
        </Reveal>
              ...
              <div className="service-item" style={{ padding: "32px 0", borderBottom: i < 3 ? "1px solid #E2E8F0" : "none", transition: "padding-left .2s, border-left .2s" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "10px", flexWrap: "wrap", gap: "8px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
                    <span className="syne" style={{ fontSize: "12px", fontWeight: 800, color: "#F97316", letterSpacing: ".1em" }}>{n}</span>
                    <h3 className="syne" style={{ fontSize: "19px", fontWeight: 700, color: "#172033", letterSpacing: "-.02em" }}>{title}</h3>
                  </div>
                  <span style={{ fontSize: "11px", color: "#0B1F3A", fontWeight: 700, letterSpacing: ".06em", whiteSpace: "nowrap", background: "rgba(11,31,58,.07)", padding: "4px 10px", borderRadius: "4px" }}>{sub}</span>
                </div>
                <p style={{ fontSize: "14px", lineHeight: 1.75, color: "#64748B", marginBottom: "16px" }}>{body}</p>
                <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                  {tags.map(t => (
                    <span key={t} style={{ fontSize: "11px", fontWeight: 600, color: "#0B1F3A", background: "rgba(11,31,58,.06)", border: "1px solid rgba(11,31,58,.12)", borderRadius: "4px", padding: "3px 10px" }}>{t}</span>
                  ))}
                </div>
              </div>
            </Reveal>
...
const Packages = () => (
  <section
    style={{
      background:
        "linear-gradient(180deg, #f9f8f5 0%, #f5f3ed 100%)",
      padding: "110px 28px",
  ...
  >
    ...
    <div
      style={{
        ...
        borderRadius: "50%",
        background:
          "radial-gradient(circle, rgba(11,31,58,.06) 0%, rgba(11,31,58,0) 70%)",
        top: "-180px",
        left: "-150px",
        pointerEvents: "none",
      }}
    ...
    <div
      style={{
        ...
        borderRadius: "50%",
        background:
          "radial-gradient(circle, rgba(249,115,22,.07) 0%, rgba(249,115,22,0) 70%)",
        bottom: "-250px",
        right: "-180px",
        pointerEvents: "none",
      }}
    ...
    >
      ...
      >
        ...
        >
          No lock-in. Cancel anytime. Results within 30 days.
        </p>
      </Reveal>
      ...
      >
        {[
          {
            name: "GroundForce",
            target: "New to digital",
            tier: "Entry",
            desc: "Build your first real digital presence. Get found locally on Google. Start receiving leads this month.",
            setup: "$800–$1,150",
            retainer: "$150–$250",
            retainerNote: "Optional monthly retainer",
            features: [
              "Conversion landing page",
              "Google Business setup",
              "Click-to-call CTA + instant text alerts",
              "Basic local SEO",
          ...
          },
          {
            ...
            target: "Most Popular",
            tier: "Growth",
            desc: "Full website + Google Ads + SEO working together to dominate your service zone with aggressive lead generation.",
            setup: "$1,800–$2,800",
            retainer: "$500–$800",
            retainerNote: "Monthly · Ads + SEO + reporting",
            features: [
          ...
          },
          {
            ...
            tier: "Dominance",
            desc: "Multi-channel, retargeting, competitor displacement, and a dedicated account manager for regional market ownership.",
            setup: "$3,900–$6,500",
            retainer: "$1,150–$1,600",
            retainerNote: "Monthly · Full-funnel management",
            features: [
              "Multi-location infrastructure",
              "Google + Meta full-funnel",
            ...
            >
              <div
                ...
                style={{
                  height: "100%",
                  background: featured
                    ? "linear-gradient(145deg, #0B1F3A 0%, #0a3d25 100%)"
                    : "rgba(255,255,255,.92)",
                  border: featured
                    ? "1px solid rgba(249,115,22,.75)"
                    : "1px solid #e5e0d4",
                  borderRadius: "18px",
                  overflow: "hidden",
                  boxShadow: featured
                    ? "0 28px 70px rgba(11,31,58,.24)"
                    : "0 12px 35px rgba(20,30,20,.06)",
                  transform: featured
                    ? "scale(1.035)"
                    : "scale(1)",
                  position: "relative",
              ...
              >
                ...
                {featured && (
                  <div
                    style={{
                      background:
                        "linear-gradient(90deg, #F97316, #e6c75b, #F97316)",
                      color: "#0B1F3A",
                      textAlign: "center",
                      fontSize: "10px",
                      fontWeight: 900,
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <br />
                      Retainer billed monthly via Wise / Payoneer
                    </p>
                  </div>
      ...
      <Reveal>
        ...
        >
          ...
          >
            All prices in USD. 50% upfront · 50% on delivery.
            <br />
            Custom quotes available for multi-location HVAC groups.
          </p>
        </div>
      </Reveal>
    </div>
...
/* ─── PROCESS ────────────────────────────────────────────────────── */
const Process = () => (
  <section id="process" style={{ background: "#0B1F3A", padding: "100px 28px" }}>
    <div style={{ maxWidth: "860px", margin: "0 auto" }}>
      <Reveal style={{ textAlign: "center", marginBottom: "64px" }}>
        <span className="pill-label" style={{ background: "rgba(249,115,22,.15)", color: "#F97316", border: "1px solid rgba(249,115,22,.3)", display: "inline-flex" }}>How It Works</span>
      ...
      <div className="process-steps" style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        {[
         { n: "01", emoji: "📞", title: "Book a Free Strategy Call", body: "Tell us about your HVAC business and service area. No pitch deck, no pressure — just a 15-minute call to understand what you need.", tag: "Usually booked same-day", highlight: false },
        ...
        { n: "03", emoji: "📞", title: "Leads Come Directly to You", body: "Homeowners searching Google find you at the top. Every call, text, and booking goes straight to you — and our AI receptionist makes sure a ringing phone never turns into a missed job.", tag: "First leads within 30 days", highlight: false },
          ...
          <Reveal key={n} type="reveal-left" delay={i * 0.14}>
            <div className="process-step-grid" style={{
              ...
              background: highlight ? "rgba(249,115,22,.1)" : "rgba(255,255,255,.05)",
              border: highlight ? "1px solid rgba(249,115,22,.3)" : "1px solid rgba(255,255,255,.08)",
              borderRadius: "10px", overflow: "hidden",
            }}>
              <div style={{ background: highlight ? "#F97316" : "rgba(255,255,255,.06)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "6px", padding: "24px 12px" }}>
                <span style={{ fontSize: "22px" }}>{emoji}</span>
                <span className="syne" style={{ fontSize: "11px", fontWeight: 800, color: highlight ? "#0B1F3A" : "rgba(255,255,255,.35)", letterSpacing: ".08em" }}>{n}</span>
              </div>
              <div style={{ padding: "26px 28px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "8px", flexWrap: "wrap", gap: "8px" }}>
                  ...
                  <span style={{ fontSize: "10px", fontWeight: 700, color: "#F97316", background: "rgba(249,115,22,.12)", border: "1px solid rgba(249,115,22,.2)", padding: "3px 10px", borderRadius: "100px", letterSpacing: ".06em", whiteSpace: "nowrap" }}>{tag}</span>
                </div>
  ...
  <section style={{ background: "#fff", padding: "100px 28px", borderTop: "1px solid #f0f0f0" }}>
    <div style={{ maxWidth: "920px", margin: "0 auto" }}>
      <Reveal style={{ textAlign: "center", marginBottom: "56px" }}>
        ...
        <h2 className="syne" style={{ fontSize: "clamp(1.9rem,4vw,2.8rem)", fontWeight: 800, color: "#172033", marginTop: "16px", letterSpacing: "-.03em" }}>The Honest Comparison</h2>
      </Reveal>

      <Reveal type="reveal-scale">
        <div style={{ border: "1px solid #E2E8F0", borderRadius: "12px", overflow: "hidden", boxShadow: "0 4px 24px rgba(0,0,0,.05)" }}>
          <div className="compare-grid-row" style={{ background: "#0B1F3A" }}>
            {["What you need", "Generic agency", "Webonrock"].map((h, i) => (
              <div key={h} className={i === 1 ? "compare-hide-mobile" : ""} style={{ padding: "16px 24px", fontSize: "11px", fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: i === 2 ? "#F97316" : "rgba(255,255,255,.5)", textAlign: i > 0 ? "center" : "left" }}>{h}</div>
            ))}
          </div>
          {[
            ["HVAC specialist, not generic", "❌ Any industry", "✅ HVAC only"],
            ...
            ["Instant text-back reporting", "❌ PDF no one reads", "✅ Texted weekly"],
            ["U.S. local SEO", "❌ Generic SEO", "✅ Geo-targeted U.S."],
            ["Results tracked as leads, not clicks", "❌ Vanity metrics", "✅ Leads counted only"],
          ].map(([need, them, us], i) => (
            <div key={i} className="compare-grid-row" style={{ background: i % 2 === 0 ? "#fff" : "#faf9f6", borderBottom: i < 6 ? "1px solid #E2E8F0" : "none" }}>
              <div style={{ padding: "14px 24px", fontSize: "13px", color: "#172033", fontWeight: 500 }}>{need}</div>
              <div className="compare-hide-mobile" style={{ padding: "14px 24px", fontSize: "13px", color: "#64748B", textAlign: "center" }}>{them}</div>
              <div style={{ padding: "14px 24px", fontSize: "13px", color: "#0B1F3A", fontWeight: 700, textAlign: "center" }}>{us}</div>
            </div>
          ))}
        </div>
      </Reveal>
      ...
      <Reveal style={{ textAlign: "center", marginTop: "40px" }}>
        <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" className="btn-gold">
          <PhIcon size={16} /> Start With a Free Strategy Call
        </a>
      </Reveal>
    </div>
  </section>
...
/* ─── FAQ ────────────────────────────────────────────────────────── */
const faqs = [
  { q: "You don't have HVAC case studies yet — why should I trust you?", a: "Fair question. We have 3 years of results building lead systems for home service businesses in India. The Google algorithm, the ad auction, the conversion principles — they're identical everywhere. What changes is local keywords and competitor landscape, and we research those before we start. You benefit from proven systems without paying the premium of a 'US-experienced' agency that's still learning what actually works." },
  { q: "You're based in India — how will we actually communicate?", a: "All project work happens via Zoom calls, email, and a shared Google Drive folder. We schedule calls around your U.S. time zone — morning or evening, whatever works for your schedule." },
  { q: "Do you have U.S. HVAC clients already?", a: "Not yet — and we say that openly on this page. We're actively expanding into the U.S. HVAC market and offering introductory pricing for our first 3 clients. That means you get a founder-level developer working directly on your project, not a junior handed the account." },
  { q: "What makes this different from a regular web design agency?", a: "Most web agencies hand you a website and walk away. We build lead pipelines — every page, ad, and the AI receptionist itself are engineered to turn a ringing phone into a booked job, including the calls that come in after hours. We don't track page views. We track leads." },
  ...
  { q: "How do I pay you?", a: "We invoice in USD and accept payment via Wise or ACH transfer — both work seamlessly from the U.S. You pay in dollars; no bank transfer paperwork on your end." },
  { q: "What exactly does the AI receptionist do?", a: "When a call goes unanswered — during a job, after hours, or on a busy day — it automatically sends the caller a text so they don't hang up and call your competitor next. It can also answer basic questions and book straight into your calendar, so leads get captured even when nobody's free to pick up." },
];
    ...
    <section style={{ background: "#fff", padding: "100px 28px", borderTop: "1px solid #f0f0f0" }}>
      <div style={{ maxWidth: "780px", margin: "0 auto" }}>
        <Reveal style={{ textAlign: "center", marginBottom: "56px" }}>
          ...
          <h2 className="syne" style={{ fontSize: "clamp(1.9rem,4vw,2.8rem)", fontWeight: 800, color: "#172033", marginTop: "16px", letterSpacing: "-.03em" }}>Questions We Get Asked</h2>
        </Reveal>
            ...
            {faqs.map(({ q, a }, i) => (
              <div key={i} style={{ borderBottom: i < faqs.length - 1 ? "1px solid #E2E8F0" : "none" }}>
                <button
                  ...
                  style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "22px 28px", background: open === i ? "#f9f8f5" : "#fff", border: "none", cursor: "pointer", color: "#172033", fontFamily: "'Inter',sans-serif", textAlign: "left", transition: "background .15s" }}
                >
                  <span style={{ fontSize: "15px", fontWeight: 600, paddingRight: "24px", color: "#172033" }}>{q}</span>
                  <span style={{ width: "28px", height: "28px", borderRadius: "50%", background: open === i ? "#0B1F3A" : "rgba(11,31,58,.1)", color: open === i ? "#fff" : "#0B1F3A", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "18px", lineHeight: "1", flexShrink: 0, transform: open === i ? "rotate(45deg)" : "none", transition: "all .2s" }}>+</span>
                </button>
                {open === i && (
                  <div style={{ padding: "0 28px 22px", fontSize: "14px", lineHeight: 1.85, color: "#64748B", background: "#f9f8f5", animation: "fadeUp .25s ease both" }}>
                    {a}
                  </div>
                )}
              </div>
      ...
      <div style={{ maxWidth: "760px", margin: "0 auto", textAlign: "center" }}>
        <Reveal>
          ...
          <p style={{ fontSize: "17px", color: "rgba(255,255,255,.6)", lineHeight: 1.75, margin: "0 auto 40px", maxWidth: "540px" }}>
            Every week without a proper digital presence is another week your competitor books the emergency AC and furnace calls that should've been yours. Let's fix that — starting today.
          </p>
          <a href={BOOKING_URL} className="btn-gold" style={{ fontSize: "16px", padding: "16px 36px" }}>
        <PhIcon size={20} /> Book Your Free Strategy Call
        </a>
        <p style={{ marginTop: "18px", fontSize: "12px", color: "rgba(255,255,255,.3)" }}>Free 15-min strategy call · No contracts · U.S. HVAC specialists</p>
        </Reveal>
      </div>
    </div>
      ...
      <div className="footer-grid">
        <div>
          <p className="syne" style={{ fontWeight: 800, fontSize: "22px", letterSpacing: "-.04em", marginBottom: "16px", color: "#fff" }}>
            webonrock<span style={{ color: "#F97316" }}>.</span>
          </p>
          <p style={{ fontSize: "13px", lineHeight: 1.8, color: "rgba(255,255,255,.4)", maxWidth: "240px" }}>
            Lead generation and AI-powered call systems for HVAC contractors across the United States.
          </p>
        </div>
        {[
